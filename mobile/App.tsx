...
type Profile = { preferredName: string; buddyName: string; age: string; difficulty: Difficulty };
...
const DEFAULT_PROFILE: Profile = { preferredName: '', buddyName: 'Mah Buddy', age: '', difficulty: 'normal' };
...
export default function App() {
  ...
  const [messages, setMessages] = useState<Message[]>([{ id: 'welcome', role: 'assistant', content: 'Hi, I’m Mah Buddy. What would you like to learn or work on?' }]);
  ...
  async function loadProfile() {
    try { const raw = await AsyncStorage.getItem(PROFILE_KEY); if (raw) setProfile({ ...DEFAULT_PROFILE, ...JSON.parse(raw) }); } catch {}
  }
  ...
  async function signIn() {
    ...
    setAuthBusy(true); const { error } = await supabase.auth.signInWithPassword({ email: email.trim(), password }); setAuthBusy(false);
  ...
  async function signUp() {
    ...
    setAuthBusy(true); const { data, error } = await supabase.auth.signUp({ email: email.trim(), password }); setAuthBusy(false);
    if (error) return Alert.alert('Sign up failed', error.message);
    if (!data.session) Alert.alert('Check your email', 'Your account was created. Complete email verification if required, then sign in.');
  }
  async function signOut() {
    if (supabase) await supabase.auth.signOut();
    setMessages([{ id: 'welcome', role: 'assistant', content: 'Hi, I’m Mah Buddy. What would you like to learn or work on?' }]);
  }
  async function sendMessage() {
    const text = input.trim(); if (!text || loading) return;
    const userMessage: Message = { id: `${Date.now()}-u`, role: 'user', content: text };
    const nextMessages = [...messages, userMessage]; setMessages(nextMessages); setInput(''); setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messages: nextMessages.map(({ role, content }) => ({ role, content })) }) });
      const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Mah Buddy could not respond right now.');
      setMessages((current) => [...current, { id: `${Date.now()}-a`, role: 'assistant', content: String(data.text || '') }]);
    } catch (error) { setMessages((current) => [...current, { id: `${Date.now()}-e`, role: 'assistant', content: error instanceof Error ? error.message : 'Connection error. Please try again.' }]); }
  ...
  if (profileOpen) return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={styles.authContainer} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><View style={styles.authCard}><Text style={styles.title}>Your profile</Text><Text style={styles.subtitle}>Mah Buddy uses these preferences to personalise learning.</Text><TextInput value={profile.preferredName} onChangeText={(v) => setProfile({ ...profile, preferredName: v })} placeholder="Preferred name" placeholderTextColor="#8a8f98" style={styles.authInput} /><TextInput value={profile.buddyName} onChangeText={(v) => setProfile({ ...profile, buddyName: v })} placeholder="Name for Mah Buddy" placeholderTextColor="#8a8f98" style={styles.authInput} /><TextInput value={profile.age} onChangeText={(v) => setProfile({ ...profile, age: v })} placeholder="Age" keyboardType="number-pad" placeholderTextColor="#8a8f98" style={styles.authInput} /><Text style={styles.preferenceLabel}>Learning difficulty</Text><View style={styles.choices}>{(['easy','normal','hard'] as Difficulty[]).map((d) => <Pressable key={d} onPress={() => setProfile({ ...profile, difficulty: d })} style={[styles.choice, profile.difficulty === d && styles.choiceSelected]}><Text style={profile.difficulty === d ? styles.choiceTextSelected : styles.choiceText}>{d[0].toUpperCase() + d.slice(1)}</Text></Pressable>)}</View><Pressable onPress={saveProfile} style={styles.primary}><Text style={styles.primaryText}>Save profile</Text></Pressable><Pressable onPress={() => setProfileOpen(false)} style={styles.secondary}><Text style={styles.secondaryText}>Back to chat</Text></Pressable></View></KeyboardAvoidingView></SafeAreaView>;

  return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><View style={styles.header}><View><Text style={styles.title}>Mah Buddy</Text><Text style={styles.subtitle}>{profile.preferredName ? `Hello, ${profile.preferredName}` : 'Your AI study buddy'}</Text></View><View style={styles.headerActions}><Pressable onPress={() => setProfileOpen(true)}><Text style={styles.action}>Profile</Text></Pressable><Pressable onPress={signOut}><Text style={styles.action}>Sign out</Text></Pressable></View></View><FlatList style={styles.list} contentContainerStyle={styles.messages} data={messages} keyExtractor={(item) => item.id} renderItem={({ item }) => <View style={[styles.bubble, item.role === 'user' ? styles.userBubble : styles.assistantBubble]}><Text style={[styles.bubbleText, item.role === 'user' && styles.userText]}>{item.content}</Text></View>} />{loading && <View style={styles.typing}><ActivityIndicator size="small" /><Text style={styles.typingText}>Mah Buddy is thinking…</Text></View>}<View style={styles.composer}><TextInput value={input} onChangeText={setInput} placeholder="Ask Mah Buddy anything…" placeholderTextColor="#8a8f98" style={styles.input} multiline maxLength={8000} /><Pressable onPress={sendMessage} disabled={!canSend} style={[styles.send, !canSend && styles.sendDisabled]}><Text style={styles.sendText}>↑</Text></Pressable></View></KeyboardAvoidingView></SafeAreaView>;
}

const styles = StyleSheet.create({
  safe:{flex:1,backgroundColor:'#f7f7f8'},container:{flex:1},authContainer:{flex:1,justifyContent:'center',padding:20},authCard:{backgroundColor:'#fff',borderRadius:22,padding:22,borderWidth:StyleSheet.hairlineWidth,borderColor:'#e0e2e6'},center:{flex:1,alignItems:'center',justifyContent:'center',gap:10},header:{paddingHorizontal:18,paddingVertical:14,borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:'#d9dce1',flexDirection:'row',justifyContent:'space-between',alignItems:'center',backgroundColor:'#fff'},headerActions:{flexDirection:'row',gap:14},action:{color:'#60656d',fontSize:13},title:{fontSize:20,fontWeight:'700',color:'#17181b'},subtitle:{marginTop:2,fontSize:12,color:'#70757d'},authInput:{height:48,borderWidth:1,borderColor:'#d7d9de',borderRadius:13,paddingHorizontal:14,marginTop:12,color:'#17181b',backgroundColor:'#fff'},primary:{marginTop:16,height:48,borderRadius:14,backgroundColor:'#17181b',alignItems:'center',justifyContent:'center'},primaryText:{color:'#fff',fontWeight:'700'},secondary:{marginTop:10,height:46,borderRadius:14,borderWidth:1,borderColor:'#d7d9de',alignItems:'center',justifyContent:'center'},secondaryText:{color:'#17181b',fontWeight:'600'},setup:{marginTop:14,color:'#8a8f98',fontSize:12,textAlign:'center'},preferenceLabel:{marginTop:18,fontSize:13,fontWeight:'600',color:'#60656d'},choices:{flexDirection:'row',gap:8,marginTop:8},choice:{flex:1,height:42,borderRadius:12,borderWidth:1,borderColor:'#d7d9de',alignItems:'center',justifyContent:'center'},choiceSelected:{backgroundColor:'#17181b',borderColor:'#17181b'},choiceText:{fontSize:12,color:'#60656d'},choiceTextSelected:{fontSize:12,color:'#fff',fontWeight:'700'},list:{flex:1},messages:{padding:16,gap:10},bubble:{maxWidth:'88%',paddingHorizontal:14,paddingVertical:11,borderRadius:17},assistantBubble:{alignSelf:'flex-start',backgroundColor:'#fff',borderWidth:StyleSheet.hairlineWidth,borderColor:'#e0e2e6'},userBubble:{alignSelf:'flex-end',backgroundColor:'#17181b'},bubbleText:{fontSize:15,lineHeight:22,color:'#24262b'},userText:{color:'#fff'},typing:{flexDirection:'row',alignItems:'center',gap:8,paddingHorizontal:18,paddingBottom:8},typingText:{fontSize:12,color:'#70757d'},muted:{fontSize:13,color:'#70757d'},composer:{margin:12,padding:8,borderRadius:22,borderWidth:StyleSheet.hairlineWidth,borderColor:'#d7d9de',backgroundColor:'#fff',flexDirection:'row',alignItems:'flex-end'},input:{flex:1,minHeight:42,maxHeight:120,paddingHorizontal:10,paddingTop:10,paddingBottom:8,fontSize:15,color:'#17181b'},send:{width:40,height:40,borderRadius:20,alignItems:'center',justifyContent:'center',backgroundColor:'#17181b'},sendDisabled:{opacity:0.35},sendText:{color:'#fff',fontSize:23,lineHeight:25,fontWeight:'700'}
});
