...
export async function GET(request:Request){
  if(!supabaseUrl||!serviceKey||!adminEmail) return NextResponse.json({error:"Admin dashboard is not configured."},{status:503});
  const auth=request.headers.get("authorization");
  const token=auth?.startsWith("Bearer ")?auth.slice(7):null;
  if(!token) return NextResponse.json({error:"Unauthorized"},{status:401});
  const supabase=createClient(supabaseUrl,serviceKey,{auth:{persistSession:false,autoRefreshToken:false}});
  const {data:{user},error:userError}=await supabase.auth.getUser(token);
  if(userError||!user?.email||user.email.toLowerCase()!==adminEmail) return NextResponse.json({error:"Access denied"},{status:403});
